...
3. **匯入補充** — 這是「用每天的測試補充智庫」的主要入口，見下方說明
4. **複習** — 把智庫變回測驗：抽題翻牌，自評「記得／還不熟」，「還不熟」的條目下次會優先出現，可依分類、重點、最近新增等條件出題

### 每天用測試問答補充智庫

- **自動掃描本機其他工具**：開啟頁面時（可於設定關閉）自動掃描同一個瀏覽器裡其他工具存下的問答，例如 `english-quiz-tool.html` 與 `toeic700-learning-plan.html` 累積的題庫，發現新問答會在智庫頁面跳出「發現 N 筆還沒整理的測試問答」。選擇題會自動轉成「正解 ＋ 解析 ＋ 選項」的知識條目，勾選後加入智庫或略過（略過的不會再出現）。需與那些工具在同一個網域開啟才掃得到（同一個 GitHub Pages 站台，或都用本機同一個伺服器開啟）
- **貼上／上傳**：支援 `Q: / A:`、`問：/ 答：`、`1. 題目＋內容`、Markdown 標題、JSON 陣列等格式，也可上傳 `.txt` / `.md` / `.json` / `.csv`，解析後先預覽再入庫
- **AI 整理**：格式太亂就按「用 AI 整理成知識條目」，Claude 會把一整段測試紀錄整理成獨立可讀的問答條目並自動分類、下標籤
- 入庫時自動比對重複問題，同一題不會重複累積

### 備份與 API Key

- 右上角 💾 可匯出 JSON 備份、匯出 Markdown（依分類整理），或還原 JSON；智庫只存在這台瀏覽器，換裝置或清除瀏覽器資料就會消失，建議定期匯出
- 「問 AI」、「AI 整理」、「AI 補強答案」需要 Anthropic API Key（右上角齒輪設定，與本 repo 其他 AI 工具共用同一組 Key，只存在瀏覽器本機）。**沒有 Key 也能完整使用**手動新增、掃描匯入、貼上匯入、搜尋與複習
