...
預設是**本機模式**——資料存在瀏覽器 `localStorage`（`mkb_entries_v1`），不經過任何伺服器，但綁定這台裝置的這個瀏覽器，清除瀏覽器資料就會消失。

部署後端後可切換為**雲端同步模式**，手機與電腦共用同一份智庫：

- `marketing-kb/worker.js` — Cloudflare Worker：帳號認證（PBKDF2 雜湊、90 天 Session Token）＋ 同步 API ＋ Claude API 代理
- `marketing-kb/schema.sql` — Cloudflare D1 結構（`kb_users`、`kb_sessions`、`kb_entries`）
- `marketing-kb/DEPLOY.md` — 完整部署教學（D1 → Worker → 綁定 → 連線）

連線方式：開啟工具 → 點右上角「本機模式」→ 貼上 Worker 網址 → 註冊或登入。第一次登入是**合併**，本機既有條目會上傳，不會被洗掉。

同步規則：

- **本機優先** — 所有操作先寫進瀏覽器再背景同步（約 1.5 秒後），網路慢不會卡住操作；離線照常使用，恢復連線自動補同步
- **同步時機** — 新增／編輯／刪除／複習後自動同步、開啟頁面時同步、切回分頁超過一分鐘同步，也可手動「立即同步」
- **衝突處理** — 同一條在兩台裝置都改過，以最後修改時間較新的為準
- **刪除會同步** — 刪除動作會傳到其他裝置；若刪除後另一台又編輯過同一條（時間較新），該條會復活

### 備份與 API Key

- 右上角 💾 可匯出 JSON 備份、匯出 Markdown（依分類整理），或還原 JSON。即使用了雲端同步，仍建議定期匯出備份
- 圖片辨識、問 AI、AI 整理、AI 補強答案需要 Anthropic API Key，兩種方式擇一：
  - 各裝置在右上角齒輪自備 Key（與本 repo 其他 AI 工具共用同一組，只存在瀏覽器本機）
  - 在 Worker 設定 `ANTHROPIC_API_KEY` Secret，登入後所有裝置共用，手機不必自備 Key
- **沒有 Key 也能完整使用**內建題庫、手動新增、貼上匯入、搜尋、複習與雲端同步
